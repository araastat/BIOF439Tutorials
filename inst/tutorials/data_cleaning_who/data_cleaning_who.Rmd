...
cleaned_data <- who %>% 
  gather(key = variable, value = counts, ____________)
```
...
cleaned_data <- who %>% 
  ...
  separate(variable, into = c('diagnosis', 'gender_age'), sep = ____) %>% 
...
cleaned_data <- who %>% 
  gather(key = variable, value = counts, starts_with('new')) %>% 
  separate(variable, into = c('diagnosis', 'gender_age'), sep = '_') %>% 
...
The new `pivot_longer` function in **tidyr** is quite flexible, and allows you to 
